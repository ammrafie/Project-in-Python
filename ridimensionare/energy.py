""" 
Running this module in isolation, the energy of an image will be visualized as 
a grayscale heat map, with brighter spots representing pixels:
...
def energy_at(pixels, x, y):
    ...
    """
    This function computes the energy of the image at the given (x, y) position.
    ...
    Expected returnValue: A single number representing the energy at that point.
    """
...
def compute_energy(pixels):
    ...
    """
    This functions computes the energy of the image at every pixel using the 
    `energy_at` function, which actually computes the energy at any single 
    position. The input is given as a 2D array of colors, and the output should 
    be a 2D array of numbers, each representing the energy value at the 
