...
Removing the lowest-energy seam from an image iteratively, the size of the image
can be reduced (in one dimension) by multiple pixels.
...
def remove_seam_from_image(image, seam_xs):
    ...
    Expected return value: The 2D grid of colors. The grid will be smaller
    than the input by one element in each row, but will have the same number 
    of rows.
    """
...
def remove_n_lowest_seams_from_image(image, num_seams_to_remove):
    ...
    Expected return value: The 2D grid of colors. The grid will be smaller than 
    the input by `num_seams_to_remove` elements in each row, but will have the 
    same number of rows.
    """
    ...
    for i in range(num_seams_to_remove):
        ...
        seam_xs, _ = compute_vertical_seam(energy_data)
